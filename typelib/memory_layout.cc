...
void MemoryLayout::pushMemcpy(size_t size, std::vector<uint8_t> const& init_data)
{
    ...
    else
    {
        ...
        init_ops.push_back(size);
        init_ops.insert(init_ops.end(), init_data.begin(), init_data.end());
    }
}
...
MemoryLayout::Ops MemoryLayout::simplifyInit() const
{
    ...
    size_t current_op = FLAG_INIT_SKIP, current_op_count = 0;
    ...
    for (; it != end; ++it)
    {
        ...
        else if (op == FLAG_INIT_CONTAINER || op == FLAG_INIT_REPEAT)
        {
            result.push_back(op);
            result.push_back(*(++it));
        }
        ...
        {
            throw std::logic_error("found unknown InitOperations code in layout");
        }
    }
...
bool MemLayout::Visitor::visit_ (Enum    const& type)
{
    ...
    if (!type.values().empty())
    {
        ...
        std::copy(&init_value, &init_value + 1, init_data.begin());
    }
